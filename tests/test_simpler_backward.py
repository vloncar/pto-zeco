"""System test for the direct PTO-runtime ("simpler") AllScan backward pass.

Compiles the backward AIV kernel + orchestration shim and runs the multi-chip
reverse-ring DAG on the target platform (``--platform``) across the devices
given by ``--device``. Verifies ``(dS, dgamma)`` against the sequential golden.
"""
...
def test_simpler_allscan_backward(test_config, device_ids, dk, dv, K):
    if len(device_ids) < 2:
