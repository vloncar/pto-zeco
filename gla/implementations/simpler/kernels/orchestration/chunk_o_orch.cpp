 ...
 *       [4]=s_snap [N,dk,dv] IN, [5]=tril [C,C] IN, [6]=o [N*C,dv] OUT,
 *       [7]=config int64[4]={C,dk,dv,N} IN.
 */
...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const L2TaskArgs &orch_args) {
    ...
    // q/k/g_cs/q_eff/k_eff are [C,dk]; v/o/inter/intra are [C,dv]; Aqk is [C,C];
    // the snapshot state S_n is [dk,dv] — three widths when dk != dv.
    ...
    TensorCreateInfo inter_ci(cdv_shape, 2, DataType::FLOAT32);
    TensorCreateInfo aqk_ci(cc_shape, 2, DataType::FLOAT32);
    TensorCreateInfo aqkm_ci(cc_shape, 2, DataType::FLOAT32);
    TensorCreateInfo intra_ci(cdv_shape, 2, DataType::FLOAT32);
    ...
    for (int n = 0; n < N; n++) {
        ...
        Tensor o_n = o.view(cdv_flat, dv_off);

        // prep -> q_eff [C,dk], k_eff [C,dk]
        L0TaskArgs p_prep;
        ...
        // inter = q_eff @ S_n  (NN): [C,dk]@[dk,dv] = [C,dv], M=C, N=dv, Kc=dk
        L0TaskArgs p_inter;
        ...
        p_inter.add_scalar(static_cast<uint64_t>(dv));  // N
        p_inter.add_scalar(static_cast<uint64_t>(dk));  // Kc
        TaskOutputTensors inter_outs = rt_submit_aic_task(FUNC_MM, p_inter);
        const Tensor &inter = inter_outs.get_ref(0);

        // Aqk = q_eff @ k_eff^T  (NT): [C,dk]@[dk,C] = [C,C], M=C, N=C, Kc=dk
        L0TaskArgs p_aqk;
        ...
        p_aqk.add_scalar(static_cast<uint64_t>(dk));  // Kc
        ...
        // intra = Aqk_m @ v  (NN): [C,C]@[C,dv] = [C,dv], M=C, N=dv, Kc=C
        L0TaskArgs p_intra;
        ...
        p_intra.add_scalar(static_cast<uint64_t>(dv));  // N
        ...
        p_out.add_scalar(static_cast<uint64_t>(dv));  // cols
